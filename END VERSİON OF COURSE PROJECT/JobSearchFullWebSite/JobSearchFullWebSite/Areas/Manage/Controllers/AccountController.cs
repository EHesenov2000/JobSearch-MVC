...
namespace JobSearchFullWebSite.Areas.Manage.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            AppUser user = _context.Users.Include(x=>x.Admin).Include(x => x.Candidate).Include(x => x.Employer).FirstOrDefault(x => x.UserName == User.Identity.Name);
            if (user != null)
            {
                if (user.Candidate != null || user.Employer != null)
                {
        ...
        [Authorize(Roles = "SuperAdmin")]
        public IActionResult GetAdmins(string search,int page = 1)
        {
            if (page < 1)
            {
                page = 1;
            ...
            if (search!=null)
            {
                ViewBag.TotalPageCount = Math.Ceiling(_context.Users.Where(x => x.UserStatus == Enums.UserStatus.Admin && x.UserName.ToUpper().Contains(search.ToUpper())).Count() / 6m);
            }
            else
            {
                ViewBag.TotalPageCount = Math.Ceiling(_context.Users.Where(x => x.UserStatus == Enums.UserStatus.Admin).Count() / 6m);
            }
            List<AppUser> users;
            if (search!=null)
            {
                users = _userManager.Users.Where(x => x.UserStatus == Enums.UserStatus.Admin && x.UserName != User.Identity.Name && x.UserName.ToUpper().Contains(search.ToUpper())).Skip((page - 1) * 6).Take(6).Include(X => X.Admin).ToList();
            }
            else
            {
                users = _userManager.Users.Where(x => x.UserStatus == Enums.UserStatus.Admin && x.UserName != User.Identity.Name).Skip((page - 1) * 6).Take(6).Include(X => X.Admin).ToList();
            }
            return View(users);
        }
