...
namespace JobSearchFullWebSite.Areas.Manage.Controllers
{
    ...
    [Authorize(Roles = "Admin,SuperAdmin")]
        public IActionResult Index(string search,int page = 1)
        {
            ...
            if (search!=null)
            {
                ViewBag.TotalPage = Math.Ceiling(_context.Jobs.Where(x=>x.IsDeleted==false && x.Name.ToUpper().Contains(search.ToUpper())).Count() / 6m);
            }
            else
            {
                ViewBag.TotalPage = Math.Ceiling(_context.Jobs.Where(x => x.IsDeleted == false).Count() / 6m);
            }
            List<Job> jobs;
            if (search != null)
            {
                jobs = _context.Jobs.Include(x => x.JobImages).Include(x => x.JobCategory).Include(x => x.RequiredLanguages).Where(x => x.IsDeleted == false && x.Name.ToUpper().Contains(search.ToUpper())).Skip((page - 1) * 6).Take(6).ToList();
            }
            else
            {
                jobs = _context.Jobs.Include(x => x.JobImages).Include(x => x.JobCategory).Include(x => x.RequiredLanguages).Where(x => x.IsDeleted == false).Skip((page - 1) * 6).Take(6).ToList();
            }
            return View(jobs);
        }
    ...
    [Authorize(Roles = "Admin,SuperAdmin")]
        ...
        {
            Job job = _context.Jobs.Include(x=>x.RequiredLanguages).Include(x=>x.JobImages).Include(x=>x.JobCategory).Include(x=>x.Employer).Include(x=>x.City).Include(x=>x.Applies).FirstOrDefault(x => x.Id == id);
    ...
    [Authorize(Roles = "Admin,SuperAdmin")]
        public IActionResult ChangeStatus(int id,ApplyStatus applyStatus)
        {
            Job job = _context.Jobs.FirstOrDefault(x => x.IsDeleted == false && x.ApplyStatus!=Enums.ApplyStatus.UserReject && x.Id==id);
            if (job == null) return RedirectToAction("index");
        ...
        [Authorize(Roles = "Admin,SuperAdmin")]
        public IActionResult ChangeFeatured(int id,bool isFeatured)
        {
