...
namespace JobSearchFullWebSite.Areas.Manage.Controllers
{
    ...
    [Authorize(Roles = "Admin,SuperAdmin")]
        public IActionResult Index(string search,int page=1)
        {
            ...
            if (search!=null)
            {
                ViewBag.TotalPage = Math.Ceiling(_context.Testimonials.Where(x=>x.Name.ToUpper().Contains(search.ToUpper())).Count() / 6m);
            }
            else
            {
            ...
            if (search != null)
            {
                return View(_context.Testimonials.Where(x => x.Name.ToUpper().Contains(search.ToUpper())).Skip((page - 1) * 6).Take(6).ToList());
            }
